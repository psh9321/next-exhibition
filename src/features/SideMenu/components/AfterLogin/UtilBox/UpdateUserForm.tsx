...
export const UpdateUserForm = ({ cancelCallback } : UPDATE_USER_FORM) => {
    ...
    const { register, control, setValue, getValues, handleSubmit, reset } = useForm({
        ...
        defaultValues : {
            name : user?.name??"",
            isProfileImg : user?.isProfileImg??false,
            profileImg : user?.isProfileImg ? `${process.env.NEXT_PUBLIC_FILE_DIRECTORY}/${user?.id}/profile.jpeg?v=${Date.now()}` : "",
            formFile : null,
        }
    });
    ...
    function SetProfileImgCallback(e : React.ChangeEvent<HTMLInputElement>) {
        ...
        if(!getValues("isProfileImg")) setValue("isProfileImg", true, {shouldDirty : true});
        setValue("formFile", data);
        setValue("profileImg",imgSrc, {shouldDirty : true});
    }

    function ResetProfileImgCallback() {
        if(!getValues("isProfileImg")) return
        setValue("profileImg","", {shouldDirty : true});
        setValue("isProfileImg", false, {shouldDirty : true});
        setValue("profileImg", "", {shouldDirty : true});
    }
    ...
    async function SubmitCallback(params : USER_UPDATE_ITEM) {
        ...
        await SessionUpdateCallback(newInfo);

        reset({
            ...newInfo,
            profileImg : newInfo["isProfileImg"] ? `${process.env.NEXT_PUBLIC_FILE_DIRECTORY}/${user?.id}/profile.jpeg?v=${Date.now()}` : ""
        })
    }
                ...
                <form onSubmit={handleSubmit(SubmitCallback)} className="flex flex-col justify-center items-end">
                    <ul className="inline-block w-[200px] mx-auto space-y-[20px]">
                        <li>
                            ...
                            <input maxLength={7} onInput={OnValueLimit} {...register("name")} defaultValue={user?.name} placeholder="변경할 유저 이름을 입력해주세요." type="text" id="userName" className="block w-full p-[5px_10px] border rounded-[5px] outline-none [&::placeholder]:text-[0.75rem]" />
                            <p className="h-[19px] mt-[6px] text-[#ff5c8a] text-[0.8rem]">{errors.name && `* ${errors.name.message}`}</p>
                        </li>
                        <li className="flex flex-wrap items-center gap-x-[15px]">
                            ...
                            <div className="flex gap-[5px] [&>*>svg]:size-[20px]">
                                <label className="cursor-pointer" htmlFor="userProfileImg">
                                    { getValues("isProfileImg") ? <RefreshCcw/> : <ImagePlus/> }
                                </label>
                                { getValues("isProfileImg") && <button onClick={ResetProfileImgCallback} type="button"><ImageOff/></button> }
                            </div>
                            <div className="relative w-[200px] h-[240px] mt-[10px] text-center">
                                {
                                    getValues("isProfileImg") ? 
                                    <Image className="border border-basic-color rounded-[10px]" loading="eager" src={getValues("profileImg") as string} alt="유저 이미지" sizes="100%" fill unoptimized />
                                    :
                                    <CircleUserRound className="inline-block mt-[20px]" size={150}/>
                                }
                                
                            </div>
                            <input onChange={SetProfileImgCallback} type="file" id="userProfileImg" accept="image/*" hidden/>
                        </li>
                    </ul>
