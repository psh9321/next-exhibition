...
export const UpdateUserForm = ({ cancelCallback } : UPDATE_USER_FORM) => {
    ...
    const [ isProfileImg, SetIsProfileImg ] = useState(user?.isProfileImg??false);
    ...
    return (
        <>
            ...
            <article className="relative w-full p-[10px_20px]">
                ...
                <form onSubmit={SubmitCallback} className="flex flex-col justify-center items-end">
                    <ul className="inline-block w-[200px] mx-auto space-y-[20px]">
                        <li>
                            ...
                            <input maxLength={7} onInput={OnValueLimit} defaultValue={user?.name} placeholder="변경할 유저 이름을 입력해주세요." type="text" id="userName" className="block w-full p-[5px_10px] border rounded-[5px] outline-none [&::placeholder]:text-[0.75rem]" />
                        ...
                        <li className="flex flex-wrap items-center gap-x-[15px]">
                            ...
                            <div className="flex gap-[5px] [&>*>svg]:size-[20px]">
                                <label className="cursor-pointer" htmlFor="userProfileImg">
                                    { isProfileImg ? <RefreshCcw/> : <ImagePlus/> }
                                </label>
                                { isProfileImg && <button onClick={ResetProfileImgCallback} type="button"><ImageOff/></button> }
                            </div>
                            <div className="relative w-[200px] h-[240px] mt-[10px] text-center">
                                {
                                    isProfileImg ? 
                                    <Image className="border border-basic-color rounded-[10px]" loading="eager" src={`${profileImgSrc}`} alt="유저 이미지" sizes="100%" fill unoptimized />
                                    :
                     ...
                     {/* ${isDirty ? "bg-main-color border-main-color" : "bg-basic-color border-basic-color"} */}
                    <button type="submit" className={`inline-block mt-[15px] p-[5px_20px] text-[#fff] border rounded-[5px] bg-main-color border-main-color`}>수정</button>
                </form>
