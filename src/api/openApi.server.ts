...
export async function API_EXHIBITION_LIST_SERVER(searchParams : SEARCH_PARAMS) {
    try {   
        
        const queryString = new URLSearchParams({...searchParams}).toString();

        const baseUrl = `realm2?serviceKey=${process.env["NEXT_EXHIBITION_API_KEY"]}&${queryString}`;
...
export const API_EXHIBITION_DETAIL = cache(async (seq : string) => {

    try {

        const baseUrl = `detail2?serviceKey=${process.env["NEXT_EXHIBITION_API_KEY"]}&seq=${seq}`;
